...
export const DebugPanel = ({
  ...
  return (
    <div
      ...
      className={cn(
        "flex size-full h-100 flex-col items-center justify-center theme-injected",
        className
      )}
    >
      <div className="mb-28 grid content-start justify-items-center gap-6 text-center">
        <span className="after:to-foreground relative max-w-[14ch] text-xs uppercase leading-tight opacity-40 after:absolute after:left-1/2 after:top-full after:h-16 after:w-px after:bg-linear-to-b after:from-transparent after:content-['']">
          {title}
        </span>
      </div>
...
export const InteractiveDebugPanel = ({
  ...
  return (
    <div
      className={cn(
        "z-99 left-4 top-4 font-mono text-sm text-destructive",
        className
      )}
    >
      {"{"}
