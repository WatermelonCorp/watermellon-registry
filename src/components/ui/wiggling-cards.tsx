...
function WigglingCardItem({ card, index, x }: WigglingCardItemProps) {
...
export function WigglingCards({ cards }: { cards?: CardData[] }) {
  ...
  const handleDragEnd = (_: React.PointerEvent | MouseEvent | TouchEvent, info: PanInfo) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="w-[360px]">
        ...
        >
          {data.map((card, i) => (
            <WigglingCardItem key={card.id} card={card} index={i} x={x} />
          ))}
        </motion.div>
      </div>
