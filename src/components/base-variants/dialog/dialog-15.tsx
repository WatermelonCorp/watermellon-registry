...
const Dialog15 = () => {
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-8 shadow-xl">
        <DialogHeader className="text-center">
          <DialogTitle className="text-xl text-zinc-900 dark:text-zinc-100">Invite new members</DialogTitle>
        </DialogHeader>
        <form className="flex gap-4 max-sm:flex-col" onSubmit={e => { e.preventDefault(); /* handle invite */ }}>
          <div className="grid gap-3">
            <Label htmlFor="invite-name">Name</Label>
            <Input id="invite-name" name="invite-name" placeholder="Full name" required />
            <Label htmlFor="invite-email" className="mt-2">Email</Label>
            <Input type="email" id="invite-email" name="invite-email" placeholder="name@email.com" required />
          </div>
          <Button type="submit" className="sm:self-end bg-zinc-800 text-white hover:bg-zinc-700 focus-visible:ring-zinc-800 dark:bg-zinc-700 dark:hover:bg-zinc-600 dark:focus-visible:ring-zinc-600 rounded-lg">
            Send Invite
          </Button>
        </form>
            ...
            <li key={index} className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 max-[420px]:w-50">
                ...
                <div className="flex flex-1 flex-col overflow-hidden">
                  ...
                  <span className="text-zinc-500 dark:text-zinc-400 truncate text-sm">{item.mail}</span>
                </div>
              </div>
              <Button
                size="sm"
                className="bg-zinc-200 text-zinc-900 hover:bg-zinc-300 focus-visible:ring-zinc-400 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700 dark:focus-visible:ring-zinc-600 rounded-lg"
              >
